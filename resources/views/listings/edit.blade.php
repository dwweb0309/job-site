        ...
        <div class="w-full md:w-1/2 py-24 mx-auto">
            ...
            <form action="{{ route('listings.update', $listing->slug) }}" id="payment_form" method="post" enctype="multipart/form-data"
                ...
                <div class="mb-4 mx-2">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <x-input id="apply_link" type="text" class="block mt-1 w-full" name="apply_link"
                            :value="old('apply_link') ?? $listing->apply_link" />
                    </div>
                </div>

                <div class="mb-4 mx-2">
                    <x-label for="tags" value="Tags (separate by comma)" />
                    <x-input id="tags" class="block mt-1 w-full" type="text" name="tags" :value="old('tags') ?? $listing->tags->implode('name', ', ')" />
                </div>
                ...
                <div class="mb-4 mx-2">
                    ...
                    item_text="name" name="currency_code" :value="old('currency_code')" />
                </div>
                        ...
                        <div class="relative flex items-start">
                            <div class="flex h-5 items-center">
                                <input type="checkbox" id="inperson_allowed" name="inperson_allowed"
                                    ...
                                    @if(old('inperson_allowed') ?? $listing->inperson_allowed) checked @endif>
                            </div>
                        ...
                        <div class="relative flex items-start">
                            <div class="flex h-5 items-center">
                                <input type="checkbox" id="remote_allowed" name="remote_allowed"
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-offset-0 focus:ring-indigo-200 focus:ring-opacity-50"
                                    @if(old('remote_allowed') ?? $listing->remote_allowed) checked @endif>
                            </div>
                            <div class="ml-3 text-sm">
                        ...
                        <div class="relative flex items-start">
                            <div class="flex h-5 items-center">
                                <input type="checkbox" id="hybrid_allowed" name="hybrid_allowed"
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-offset-0 focus:ring-indigo-200 focus:ring-opacity-50"
                                    @if(old('hybrid_allowed') ?? $listing->hybrid_allowed) checked @endif>
                            </div>
                            <div class="ml-3 text-sm">
